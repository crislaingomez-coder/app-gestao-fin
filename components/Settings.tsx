                    ...
                    <div>
                        ...
                        <div className="flex gap-3 text-xs text-gray-500 mt-1.5 font-medium">
                            <span className="bg-white px-2 py-1 rounded-md border border-gray-200">
                              Melhor dia: <strong className="text-gray-700">{card.bestDay}</strong>
                            </span>
                            <span className="bg-white px-2 py-1 rounded-md border border-gray-200">
                              Vence dia: <strong className="text-gray-700">{card.dueDay}</strong>
                            </span>
                        </div>
                    </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        ...
        <form onSubmit={handleCategorySubmit} className="flex gap-2 mb-6">
            <input 
                ...
                className="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm 
                           ...
                           outline-none bg-white text-gray-900 transition-all uppercase"
            />
            <button 
                type="submit" 
                disabled={!newCategory.trim()}
                className="bg-orange-500 text-white px-5 rounded-2xl font-bold hover:bg-orange-600 
                           disabled:opacity-50 shadow-md shadow-orange-500/20 transition-all active:scale-95"
            >
                Add
            </button>
        </form>
        ...
        <div className="flex flex-wrap gap-2">
            {categories.map(cat => (
                <div 
                  key={cat} 
                  className="group flex items-center gap-2 bg-gray-50 pl-4 pr-2 py-2 rounded-xl 
                             border border-gray-200 text-sm font-medium text-gray-700 
                             hover:border-orange-200 hover:bg-orange-50 transition-colors"
                >
                    <span>{cat}</span>
                    <button 
                        type="button"
                  ...
                  <form onSubmit={handleCardSubmit} className="space-y-4">
                      ...
                      <div>
                          ...
                          <input 
                            ...
                            className="w-full p-3.5 border border-gray-200 rounded-2xl 
                                       focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 
                                       outline-none bg-white text-gray-900 font-bold uppercase"
                      ...
                      <div className="flex gap-4">
                        <div className="flex-1">
                            ...
                            <input 
                              ...
                              className="w-full p-3.5 border border-gray-200 rounded-2xl 
                                         focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 
                                         outline-none bg-white text-gray-900 text-center font-bold"
                            />
                        </div>
                      ...
                      <div className="pt-4">
                          <button 
                            type="submit" 
                            className="w-full p-4 bg-blue-600 text-white rounded-2xl font-bold 
