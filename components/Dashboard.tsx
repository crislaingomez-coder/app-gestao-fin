...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const generalData = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      categoryDataMap.set(
        t.category,
        (categoryDataMap.get(t.category) || 0) + amount
      );

      if (t.cardId) {
        const cardName =
          cards.find(c => c.id === t.cardId)?.name || 'Desconhecido';
        cardDataMap.set(
          cardName,
          (cardDataMap.get(cardName) || 0) + amount
        );
      }
    });
  ...
  const monthlyData = useMemo(() => {
    ...
    monthTransactions.forEach(t => {
      categoryDataMap.set(
        t.category,
        (categoryDataMap.get(t.category) || 0) + t.amount
      );
    });
    ...
    monthTransactions.forEach(t => {
      if (t.cardId) {
        ...
        cardDataMap.set(
          cardName,
          (cardDataMap.get(cardName) || 0) + val
        );
      }
    });
  ...
  const activeData =
    viewMode === 'general' ? generalData : monthlyData;
